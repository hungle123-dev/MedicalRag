...
- The first locked evidence preflight stopped before generation when one X1 abstract was too short for its matched slot. Protocol v1.6/v10 expands only the frozen lower-ranked X1 candidate pool to top-30. Dev-v10 evidence is byte-equivalent at arm level to v9 for all 80 questions, all generation/judging reused cache, and the negative result is unchanged. Full eval preflight must pass before the single locked generation starts.
