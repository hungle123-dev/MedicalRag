...
- B3/G2/X1/X2 use identical IDs, generator, prompt, output limit and retry policy. Per question, all arms match B3's actual whitespace-word count. X1 matches graph slot count/length with lower-ranked text. X2 uses structurally valid PrimeKG paths sampled away from linked target nodes, matched on hop count and nearest verbalized length.
...
- Confirmatory claims require paired statistics and must survive B3, X1 and X2 in ITT plus a complete-X2 sensitivity analysis. API/quota failures remain in the population with conservative scores; they cannot silently change it.
