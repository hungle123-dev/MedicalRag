...
class GroqJudge:
    ...
    def __init__(self, root: Path, model: str = "openai/gpt-oss-120b"):
        self.root, self.model = root, os.getenv("GROQ_JUDGE_MODEL", model)
        self.key = os.getenv("GROQ_API_KEY")
        if not self.key: raise RuntimeError("GROQ_API_KEY is not configured")
        self.cache = root / "artifacts/model_cache/groq_judge"; self.cache.mkdir(parents=True, exist_ok=True)
