...
def main() -> None:
    ...
    for question_id in ids:
        ...
        for model in args.models:
            ...
            correctness = judge.evaluate(correctness_input(row["question"], row["answer"], generation.answer))
    ...
    for model in args.models:
        ...
        aggregates[model] = {
            ...
            "latency_ms_mean": round(sum(record["latency_ms"] for record in subset) / len(subset), 1),
        }
    ranking = sorted(args.models, key=lambda model: (
        -aggregates[model]["citation_integrity_rate"],
        -(aggregates[model]["correctness_mean_0_2"] + aggregates[model]["completeness_mean_0_2"]),
        aggregates[model]["unsupported_claim_rate_mean"], aggregates[model]["latency_ms_mean"], model,
    ))
    result = {"track": "exploratory_frozen_dev_generator_calibration", "question_ids": ids,
              "models": args.models, "judge_model": judge.model,
              "selection_rule": "citation integrity, then correctness+completeness, unsupported claims, latency",
