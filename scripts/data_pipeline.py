...
def command_download(names: list[str]) -> None:
    ...
    manifest = [download_one(name) for name in selected]
    write_json("files.json", {"retrieved_at": datetime.now(timezone.utc).isoformat(), "files": manifest})
